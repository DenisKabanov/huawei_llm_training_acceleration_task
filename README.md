...
* Test proportion: 10%
* Additional validation during training on benchmarks: "piqa", "winogrande"
* Limit on the number of tokens generated by the model: 256
* Maximum number of training epochs: 20
* Epochs before early stopping of training (without improving the tracked metric and/or convergence of weights): 3
* Tracked metric for training early stopping: AVG Accuracy on benchmarks
* Initial learning rate: $2 \cdot 10^{-5}$
* Learning scheduler that changes the learning rate 0.5 times every 4 epochs
* L2 regularization coefficient to prevent overfitting: 0.1
