...
1) The gradient $g_t$ (the derivative of the loss function with respect to the current weight $\theta$) is calculated using the formula:
    ...
    $$g_t = \nabla_{\theta}L(\theta_t) + \lambda \cdot \theta_{t-1}$$
...
5) The stage of updating the model weights ($\eta$ — learning rate, $\epsilon$ — minimum addition so that there is no zero in the denominator):
    ...
    $$\theta_{t+1} = \theta_{t} - \eta \cdot \lambda \cdot \theta_{t} - \eta \cdot \frac{\hat{m}_t}{\sqrt{\hat{v}_t} + \epsilon} = \theta_{t} - \eta \cdot \left(\lambda \cdot \theta_{t} + \frac{\hat{m}_t}{\sqrt{\hat{v}_t} + \epsilon}\right)$$
\begin{figure}[h]
\centering
...
\begin{itemize}
    \item \textit{\textbf{The highest for the AdamW optimizer}}, since it stores two moments at once for each trainable weights (the moving average of the gradients and the moving average of the squares of the gradients). It takes up 3.28 GB with the model.
...
\begin{itemize}
    ...
    \item The \textit{\textbf{convergence of models with respect to the loss function is almost the same for optimizers}}, but not according to L2 norm of updated model's weights, they change the most with AdamW optimizer, the weakest with Muon (\textit{while their loss is almost the same}).
    ...
    \item \textit{\textbf{Muon requires less memory}} (\textit{by $\sim$20\% compared to AdamW}), as it stores only one additional matrix for weights (Momentum).
